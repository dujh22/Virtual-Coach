...
def generate_eval_script(scenario_name: str, prompt_template: str,
                         ...
                         version: int = 1, max_workers: int = 8) -> str:
    """
    模板化生成评测脚本。
    ...
    # 生成 format 参数
    format_args = ",\n            ".join(
        f'{p}=temp_data["{p}"]' for p in placeholders
    )

    # 检测比对字段：取 output 中第一个不含 "reason" 的键
    comparison_keys = []
    if eval_data and eval_data[0].get("output"):
        for key in eval_data[0]["output"]:
            if "reason" not in key.lower():
                comparison_keys.append(key)

    # 生成比对逻辑
    if comparison_keys:
        compare_conditions = " and ".join(
            f'response_json.get("{k}") == ground_truth.get("{k}")' for k in comparison_keys
        )
        error_parts = ' + ", " + '.join(
            f'"GT_{k}=" + str(ground_truth.get("{k}")) + " Pred_{k}=" + str(response_json.get("{k}"))' for k in comparison_keys
        )
    else:
        compare_conditions = "response_json == ground_truth"
        error_parts = '"预测结果与期望不一致"'

    version_module = f"v{version}"
    prompt_var_name = scenario_name
...
from data.prompt.{scenario_name}.{version_module} import {prompt_var_name}
...
def evaluate_accuracy():
    ...
    def process_sample(i):
        ...
        temp_data_prompt = {prompt_var_name}.format(
            {format_args}
        )
