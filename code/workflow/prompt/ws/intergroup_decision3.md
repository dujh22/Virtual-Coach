...
2. 沟通结果 (String，可为空) {{params.communication_result}}
   //   用户和教练沟通后的反馈，例如“感觉太累”或“还想挑战更高重量”。  

3. 用户训练诉求 (String) {{params.user_goal}}
   //   用户的目标，例如“增加力量”、“保持健康”、“塑形”。  

4. 动作名称 (String) {{params.exercise_name}}
   //   例如 "深蹲"、"卧推"。  

5. 动作设定重复次数 (Integer) {{params.set_reps}}
   //   例如 10。  

6. 质量评级点配置 (JSON) 
{{params.quality_checkpoints}}
//   说明某些质量评价点是否启用及其标准
...
3. 检查质量评价点
- 如果错误与某个质量点相关，可考虑CHANGE_QUALITY_STANDARD。  
