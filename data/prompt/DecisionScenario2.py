...
2. 动作名称 (String): {exercise_name}
3. 动作重复次数设定 (Integer): {set_reps}
...
1) 计算指标
   ...
   delta_to_target = {set_reps} - high_quality_reps
...
5) 目标肌群感受 消息（仅一句，聚焦主肌/代偿肌）
   - 根据 {exercise_name} 粗略映射主肌/代偿肌（估计不确定时，使用“目标肌群/其他部位”占位）：
...
输出：
{{"need_to_communicate": true, "message": "你主要感觉到是背部在发力，还是手臂更明显？"}}
"""